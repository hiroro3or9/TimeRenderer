...
- 並べ替え、サブタスク、記録連動、タイムブロッキングに加え、通知時刻と
  アーカイブ日付の境界テストを追加した
- `MainViewModel.Todos.cs` を 698 行まで縮小し、サブタスク、並べ替え、記録連動、
  タイムブロッキング、通知、保存・アーカイブを対象別の partial ファイルへ分割した
- 画面状態に依存しない規則を `TodoSubtaskHelper`、`TodoTimeBlockHelper`、
  `TodoReminderHelper`、`TodoArchiveHelper` へ分離した
- 特性テストの追加中に、記録漏れ補完と ToDo から開始した作業記録を取り消しても
  `RecordedTicks` だけが戻らない問題を検出した。記録時間専用の Undo/Redo を追加し、
  予定・実績と同じ操作単位で戻るよう修正した

この時点で、ToDo 周辺を含む全 149 テストが合格した。
...
### 2026-08-30 追加対応 — 記録停止と離席処理

ToDo の記録連動を整理した流れで、`MainViewModel.Away.cs` に離席検知だけでなく、
記録停止時の確認、区間分割、実績生成、ToDo 積算、Undo 登録まで同居していることを確認した。

- 離席区間の切り詰め・重複統合・差し引きを `RecordingStopHelper` へ分離した
- 元予定の実績化と追加実績の生成規則を `RecordingItemHelper` へ分離した
- 記録セッションの開始・停止を `MainViewModel.Recording.cs`、停止後の保存処理を
  `MainViewModel.RecordingStop.cs` へ移し、コマンド登録と離席検知から切り離した
- すべての開始経路で離席状態を初期化し、停止処理は例外時にもセッション状態を解除する形にした
- 重複する離席区間を自動除外の合計時間へ二重計上しないよう修正した

記録停止・離席処理の 8 テストを追加し、全 157 テストが合格している。

### 2026-08-30 テスト補強

- サブタスク削除・完了それぞれの Undo/Redo と、親の自動完了＋繰り返し次回生成を
  同じ操作単位で戻す Composite Undo のテストを追加した
- 見逃し通知の 0 件・1 件・複数件の本文、朝のまとめ通知の有効状態・日付・時刻境界、
  件数別本文、最終通知日の保存往復をテストした
- JSON 保存先をテスト用ディレクトリへ切り替えられる内部経路を追加し、実データを触らずに
  正常な保存往復、一時ファイルの消去、バックアップ復旧、全世代破損時の失敗判定をテストした

全 172 テストが合格している。

### 2026-08-30 未保護領域と操作経路の追加補強

調査後の次段階として、行数ではなく「境界条件」と「実際の操作経路」を基準に補強した。

1. `UndoManager`、未記録区間、磁石スナップ、日/週レイアウト、タイムラインレーン、
   ノートタグ、繰り返し ToDo の境界テストを追加した
2. 勤務日の自動締め・離席確認を `WorkDayPolicy`、定期予定の仮想アイテム生成を
   `RoutineOccurrencePlanner` へ分離し、日付またぎ、月末、最終曜日、除外日、休み周期をテストした
3. `MainViewModel` に本番データ、OS 監視、画面タイマーを起動しない内部初期化経路と
   `TimeProvider` を追加した。クイック追加、時間ブロック、記録漏れ補完、ToDo からの
   記録開始・停止、通知、朝のまとめ、出退勤をコマンドから Undo まで通してテストした
4. `FilePersistenceService` にテスト用保存先を渡せる内部経路を追加し、予定、ToDo、
   勤務記録、アプリ使用履歴、アーカイブの保存往復、旧形式、不正値、破損時の扱いをテストした
5. Windows 上で Release ビルドと全テストを実行する GitHub Actions を追加し、
   テストが 239 件未満へ減った場合も失敗させるようにした

全 239 テストが合格している。テストは一時フォルダだけを使い、実際の AppData へは書き込まない。
...
初期調査で優先したテスト基盤、曜日マッピング、設定の正規化・マッピング、統計、ToDo、
記録停止、保存契約は完了した。`SaveSettings` は実測の結果、デバウンス不要と判断済み。
MVVM ライブラリの全面導入も、現状では費用に見合う根拠がない。

今後は次の順で、必要になった機能へ局所的に進める。

| 順 | 内容 | 着手条件 | 完了条件 |
| --- | --- | --- | --- |
| 1 | GitHub Actions の初回実行確認 | 変更をリモートへ送ったとき | Windows の Release ビルドと 239 テストが成功する |
| 2 | WPF 操作の手動スモーク確認 | 今回の変更を配布する前 | 起動、追加、記録、Undo、終了時保存を実画面で1巡する |
| 3 | カバレッジ計測 | 次の大きな機能追加前 | 数値目標ではなく、未実行の重要分岐を一覧化する |
| 4 | OS 依存処理の薄い境界化 | 離席検知・アプリ追跡を変更するとき | Win32 呼び出し以外の状態遷移を自動テストする |
| 5 | UI ロジックの追加抽出 | ドラッグやヒットテストで不具合が出たとき | 座標計算だけを純粋関数としてテストする |

画面自動操作の大規模導入、DI コンテナ、全面的な `DateTime` 置換は先回りして行わない。
今回導入した内部初期化経路と `TimeProvider` で主要なアプリケーション経路は検証できるため、
必要な場所だけ同じ方式を広げる。
