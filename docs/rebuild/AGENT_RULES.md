...
- 予定データ保存は **700ms デバウンス＋アトミック書き込み＋バックアップ世代管理**（`guide/15`）
...
- JSON に保存される enum の数値・プロパティ名を変えない（`guide/02` の値を守る）
...
- **今回のフェーズの範囲だけ**実装する。後のフェーズの機能は、ガイドで指定された「スタブ（空実装）」だけ置く
- フェーズ終了時に：ビルドが警告0・エラー0 → テスト全件成功 → フェーズ節の「手動確認」を1巡 → 変更ファイル一覧を報告
- 迷ったら推測で仕様を足さず、ガイドの該当箇所を示して質問する
