...
- `global.json`（テストランナーを Microsoft.Testing.Platform にする。無いと `dotnet test` で TUnit が動かない）
- `App`（例外ハンドラとテーマ切り替え）、アセンブリ属性、アプリアイコン（本物が無ければ仮の ico。無いとビルドが通らない）
...
**テスト**：`ObservableObject`（同値なら通知しない、戻り値）。`dotnet test` で実行できることもここで確かめる
...
**読むガイド**：`02`（保存形式の表を含む全体）、`11`（「繰り返しの種類」の節）、`09`（「期限・今日やる・繰り返し」の節）
...
**完了条件**：JSON に出るプロパティ名と enum の数値が `02` の「保存形式」の表と一致する（派生プロパティが混ざっていない）
