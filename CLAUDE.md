...
- **提示するコミットメッセージは1つにまとめる。** 実装が済んだ後から分けるのは手間なので、
  内容が複数あっても本文の箇条書きで並べる
...
- **本文は必ず書く。** 要約の後に空行を1行入れ、`- ` の**箇条書き**で書く。
  項目数に上限は設けない（内容があるだけ並べてよい）
- 各項目は1行で収める。基本は「何をしたか」だけを並べる
- **要約から分かる理由は書かない。** 「なぜ」を添えるのは、その判断が一見おかしく見えて
  後から戻されかねないときだけにする
