...
public partial class MainViewModel
{
    ...
    public sealed record WorkDayNote(DateTime Date, string DateText, string WorkText, string Note);
    ...
    /// <summary>ふりかえりの総件数（0件でも文言を返す）</summary>
    ...
    private void RebuildNoteGroups()
    {
        ...
        [
            .. _workDayLogs
                ...
                .GroupBy(l => new DateTime(l.StartTime.Year, l.StartTime.Month, 1))
                .Select(g => new NoteMonthGroup(
                    g.Key.ToString("yyyy年M月"),
                    [.. g.Select(ToWorkDayNote)]))
        ];
    }

    /// <summary>勤務記録1件を表示用に整える</summary>
    private static WorkDayNote ToWorkDayNote(WorkDayLog log) => new(
        log.StartTime.Date,
